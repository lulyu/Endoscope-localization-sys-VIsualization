...
// 轨迹点个数
static GLint trackLength;
static GLint trackLengthNow = 0;
// 当前累计有效数据点个数
...
void drawTrack(point3d *pTrack)
{
	...
	{
		for(i=0;i<trackLengthNow-1;i=i+2)
		{ 
			if (i==0)
			{
				n1.x=pTrack[1].x-pTrack[0].x;
			...
			else if (i==trackLength-2)
			{
				...
				n2.x=pTrack[trackLengthNow-1].x-pTrack[trackLengthNow-2].x;
				n2.y=pTrack[trackLengthNow-1].y-pTrack[trackLengthNow-2].y;
				n2.z=pTrack[trackLengthNow-1].z-pTrack[trackLengthNow-2].z;
				n1 = normalizeVector(n1);
				n2 = normalizeVector(n2);
			}
			else
